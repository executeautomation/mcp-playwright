...
- **New Transport Mode: HTTP/SSE Support**: Added standalone HTTP server mode for enhanced deployment flexibility 🌐
  - `--port` flag to run server in HTTP mode (e.g., `playwright-mcp-server --port 8931`)
  ...
  - Dual endpoint support: `/sse` + `/messages` (legacy) and `/mcp` (unified)
  - Graceful shutdown with proper resource cleanup
  - DRY-compliant implementation with reusable endpoint handlers
...
- **Enhanced Deployment Options**:
  ...
  - Better integration with VS Code GitHub Copilot
...
- **Code Quality Improvements**:
  - Refactored endpoint logic to eliminate duplication (~40% code reduction)
  - Implemented DRY (Don't Repeat Yourself) principles throughout HTTP server
...
- **Documentation Updates**:
  - Comprehensive HTTP/SSE transport documentation
  - Client configuration guides (Claude Desktop, VS Code)
  - Testing guides and automated test scripts
  - Troubleshooting and FAQ sections
  - Quick reference cards for common setups
  - Refactoring notes and best practices documentation
- **Security Improvements**:
  ...
  - 🔒 Added security notification in startup message
...
- **Infrastructure Improvements**:
  ...
  - Automated testing script (`test-http-mode.sh`)
- **Version bump**: Incremented version to 1.0.7
