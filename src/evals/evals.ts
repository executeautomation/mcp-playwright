...
import type { EvalConfig } from "mcp-evals";
import { type EvalFunction, grade } from "mcp-evals";
...
const startCodegenSessionEval: EvalFunction = {
  ...
  run: async () => {
    const result = await grade(
      openai("gpt-4"),
      "Please start a new code generation session with an output path of /my/test/path, a testNamePrefix of MyPrefix, and comments enabled. Confirm the session was created successfully.",
    );
...
const end_codegen_sessionEval: EvalFunction = {
  ...
  run: async () => {
    const result = await grade(
      openai("gpt-4"),
      "Please end the code generation session with ID session123 and generate the Playwright test code",
    );
    return JSON.parse(result);
  },
};
...
const get_codegen_sessionEval: EvalFunction = {
  ...
  run: async () => {
    const result = await grade(
      openai("gpt-4"),
      "Please retrieve the code generation session details using session ID abc123.",
    );
    return JSON.parse(result);
  },
};
...
const clearCodegenSessionEval: EvalFunction = {
  ...
  run: async () => {
    const result = await grade(
      openai("gpt-4"),
      "Please clear the code generation session with the ID testSession_123 to verify removal.",
    );
    return JSON.parse(result);
  },
};
...
export const evals = [
  startCodegenSessionEval,
  end_codegen_sessionEval,
  get_codegen_sessionEval,
  clearCodegenSessionEval,
];
